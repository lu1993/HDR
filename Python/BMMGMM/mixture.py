# -*- coding: utf-8 -*-
"""
Created on Tue Dec 27 16:17:59 2016
...
class mixture(object):
    ...
    def __init__(self, n_components, init_params='wm', n_iter=100, tol=1e-3,
        ...
        self.weights = np.array([1.000000 / k for _ in range(k)])
        self.means = None
        self.covars = None
    ...
    def _do_mstep(self, x, z):
        ...
        inverse_weights = 1.0 / (weights[:, np.newaxis] + 10 * EPS+2)

        # MAP estimates with beta prior for mean and Dir prior for weights
        self.weights = ((weights + 1)/ (weights.sum() + 10 * EPS + 10) + EPS)
        self.means = (weighted_x_sum + 1) * inverse_weights
